"""EDA tinh phan Quan: ve toi thieu 3-5 bieu do PNG phuc vu barem 0.75d + bao cao.
Chay: python3 scripts/eda_quan.py (yeu cau chay clean_quan.py truoc)
"""
...
def main():
    nat = pd.read_csv(PROC / "quan_co2_quoc_gia.csv")
    sec = pd.read_csv(PROC / "quan_co2_theo_nganh.csv")
    ren = pd.read_csv(PROC / "quan_nang_luong_tai_tao.csv")
    owid_raw = pd.read_csv(
        RAW / "02_quan_phat_thai_co2" / "owid_phat_thai_co2_1750_2024.csv",
        low_memory=False, usecols=["country", "year", "co2"])
    world = owid_raw[owid_raw["country"] == "World"].sort_values("year")

    # 1. Line: CO2 toan cau (dong World truc tiep, don vi Mt)
    ...
    # 2. Horizontal bar: top 15 quoc gia 2023
    ...
    # 3. Stacked area: co cau nganh EDGAR toan cau theo thoi gian
    ...
    # 4. Scatter: CO2/nguoi vs % nang luong tai tao (2023, mau theo chau luc)
    ...
    # 5. Do phu du lieu theo nam (kiem chung 2024 thieu)
    cov = pd.DataFrame({
